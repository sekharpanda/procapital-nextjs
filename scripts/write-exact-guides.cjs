...
const rules = style.match(/\.devs\{[^}]+\}|\.devs \.chip\{[^}]+\}|\.chip\{[^}]+\}/g) || []
console.log(rules)
let guide = fs.readFileSync('src/css/guide.css', 'utf8')
// remove incomplete append and re-add full
guide = guide.replace(/\n\.devs\{[^}]+\}\n?/g, '\n')
if (!guide.includes('.devs .chip') && !guide.includes('.chip{')) {
  guide += '\n' + rules.join('\n') + '\n'
}
fs.writeFileSync('src/css/guide.css', guide, 'utf8')
console.log('guide has chip', guide.includes('chip'))
...
`
fs.writeFileSync('src/components/ExactPageClient.tsx', client, 'utf8')
